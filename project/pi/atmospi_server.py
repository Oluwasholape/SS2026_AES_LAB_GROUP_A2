...
def on_message(client, userdata, msg):
    ...
    with state_lock:
        entry = latest.setdefault(node, {"ts": 0, "metrics": {}, "metric_ts": {}})
        entry["ts"] = now
        for k, v in values.items():
            entry["metrics"][k] = v
            entry["metric_ts"][k] = now
        msg_count += 1
...
def api_latest():
    ...
    with state_lock:
        ...
        for nid, meta in NODES.items():
            ...
            # A metric only counts as "live" if it has been updated recently,
            # not merely if it was ever seen once. This is what lets a sensor
            # that has gone silent (but whose node is still sending other
            # metrics) be told apart from one that is reporting fine.
            fresh_metrics = {}
            if entry:
                for m, v in entry["metrics"].items():
                    m_ts = entry["metric_ts"].get(m, 0)
                    if now - m_ts <= ONLINE_WINDOW_S:
                        fresh_metrics[m] = v
            ...
            if online and missing and ALARM_RANK["warning"] > ALARM_RANK[alarm]:
                # a node that should be producing a reading but isn't is a
                # sensor fault, and a blind spot is worse than "everything
                # looks normal", so it is never allowed to display as normal
                alarm = "warning"
